...
eigcv <- eigcv(as(A3, "sparseMatrix"), k_max = 20, laplacian = FALSE, regularize = FALSE)
plot(eigcv)
eigcv
...
eigcv <- eigcv(as(A4, "sparseMatrix"), k_max = 20, laplacian = FALSE, regularize = FALSE)
plot(eigcv)
eigcv

fa <- vsp(A4, rank = 3, degree_normalize = FALSE)
fa
...
nested <- data |> 
  ...
  mutate(
    ...
    meddep = map(tbl_graph, netmediate, promis_eddep ~ condition, rank = 3),
    medanx = map(tbl_graph, netmediate, promis_edanx ~ condition, rank = 3)
  ) |> 
  select(-data)
...
lnm <- netmediate(tbl_graph, promis_eddep ~ condition, rank = 3, coembedding = "Z")
